...
// Store the book's title to keep track that it is being displayed in the table.
   updateLibrary() {
    this.myLibrary.forEach((book) => {
      if (!this.existingBooks.includes(book.title)) {
        const newRow = bookList.insertRow(this.rowCount);
        let cellCount = 0;
        Object.values(book).forEach((prop) => {
          ...
          if (prop === "Read" || prop === "Not Read") {
            ...
            newCell.dataset.rowNumber = this.rowCount;
          }
        ...
        removeBookButton.addEventListener("click", () => {this.removeBook(event)});
        removeBookButton.dataset.rowNumber = this.rowCount;
        ...
        if (bookList.rows[this.rowCount].cells[3].textContent === "Read") {
        ...
        readToggleButton.addEventListener("click", () => {this.constructor.changeReadStatus(event)});
        readToggleButton.dataset.rowNumber = this.rowCount;
        readToggleButton.classList.add("toggle-read-button");
        readToggleCell.appendChild(readToggleButton);

        this.rowCount += 1;
        this.existingBooks.push(book.title);
      }
    });
  }
